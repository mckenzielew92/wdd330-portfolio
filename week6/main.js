...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //if there is text inside the item field then add it tot he todo list
    ...
    addInput.addEventListener('keypress', function (e){
        //did the user press enter? If yes, continue
        if (13== e.keyCode) {
            //grab the value of the input tag
            let newItem = document.getElementById('item').value
            //if the input tag is not empty then run our function to add an item
    ...
    //create addItemTodo() in order to get add Item to work!
    function addItemTodo(text) {
        //grab the ul
        let list = document.getElementById('todo')
        //create an li
        let item = document.createElement('li')
        //set the list item 'li' to be the same as the parameter that we passed in the function
        ...
        //prepend the `li` to the `ul` PREPEND adds the item before, not after, 
        //because we want new items added to the top
